 ...
 */
public class BasicEditProfileEducation extends AppTestTemplate {
    ...
    String[] actionSeq_FillAll_WrongDate_present = new String[]{"login", "switch2Profile", "clearProfile", "addEducation_FillAll_WrongDate_Present", "education_profileSaveback2Profile", "addEducation_FillAll_WrongDate_Present_Check"};
    ...
    public Object[][] profileEditEducationAll() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "abc::sample1::hou::masters::C.M.A", "regular", ""},
                {this.accontFromXls, this.accontPSWFromXls, "12313131312::nnnn::new::Associates::A.A.S", "regular", ""},
        };
    }
    ...
    public Object[][] profileEditEducationAllDate() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "12313131312::nnnn::ho::" + this.startdate + "::" + this.enddate, "regular", ""},
                {this.accontFromXls, this.accontPSWFromXls, "abc::sample1::new::" + this.startdate + "::" + this.enddate, "regular", ""},
        };
    }
    ...
    public BasicEditProfileEducation() throws FileNotFoundException {
    }
    ...
    public void addEducationAllWrongDate(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            ...
            date = new Date();
            time = format.format(date);
            boolean boolean4assert = assertTestResult(theAction.getExeResult().toString(), targetResult); // for judge, with normal result(after internal handle, compare) AND target result mode for ignore OR regular
            if (boolean4assert == true) {
                result4XLSX = "PASS";
            }
            list4result.add(nameOfMethod + "#" + time + "#" + preRoleName + "#" + targetResult + "#" + theAction.getExeResult().toString() + "#" + result4XLSX);  // for xlsx
            ...
            System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~ Single result judgement for PageTitleException ~~~~~~~~~~~~~~~~~~~~~~~~~~");
            boolean boolean4assert = assertTestResult(e.getMessage(), targetResult); // for judge, with cause info AND target result for error
            if (boolean4assert == true) {
                result4XLSX = "PASS";
            }
            list4result.add(nameOfMethod + "#" + time + "#" + preRoleName + "#" + targetResult + "#" + e.getMessage() + "#" + result4XLSX);  // for xlsx
    ...
    public void addEducationAllWrongDate_present(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
