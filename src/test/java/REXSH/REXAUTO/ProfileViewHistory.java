 ...
 */
public class ProfileViewHistory extends AppTestTemplate {
    ...
    String[] viewHistoryBasic = new String[]{"login", "switch2Profile", "viewHistory"};
    String[] viewHistoryBasic_nodate = new String[]{"login", "switch2Profile", "viewHistory_nodate"};

    @DataProvider
    public Object[][]
    historyBasic() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "Offer::"  + this.startdate + "::" + this.enddate, "ignore", ""},
                {this.accontFromXls, this.accontPSWFromXls, "Interest::"  + this.startdate + "::" + this.enddate, "ignore", ""},
        };
    }
    ...
    history_nodate() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "Interest", "ignore", ""},
        };
    }

    public ProfileViewHistory() throws FileNotFoundException {
    }
    ...
    @Test(dataProvider = "historyBasic")
    public void historyBasic(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        ...
        try {
            ...
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.viewHistoryBasic, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.viewHistoryBasic, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
