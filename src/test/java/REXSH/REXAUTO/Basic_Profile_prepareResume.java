 ...
 */
public class Basic_Profile_prepareResume extends AppTestTemplate {
    ...
    String[] actionSeq_FillAll_WrongDate = new String[]{"login", "switch2Profile", "editProfile", "addWorkExp_FillAll_WrongDate", "work_profileSaveback2Profile"};
    String[] actionSeq_Sample = new String[]{"login", "switch2Profile", "editProfile", "addSkillOnly", "skill_profileSaveback2Profile"};

    @DataProvider
    public Object[][]
    profileEditWorkWrongDate() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "com::maj::loc::" + this.startdate + "::" + this.enddate + "::desc", "ignore", ""},

        };
    }

    @DataProvider
    public Object[][] profileEditSkill() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "JAVA", "ignore", ""},
                {this.accontFromXls, this.accontPSWFromXls, "C++", "ignore", ""},
                {this.accontFromXls, this.accontPSWFromXls, "oracle", "ignore", ""},
        };
    }


    public Basic_Profile_prepareResume() throws FileNotFoundException {
    }
    ...
    @Test(dataProvider = "profileEditSkill")
    public void addSkill(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        ...
        try {
            //  BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);
            //   theAction.comOperationInAND(driver, actionSeq_Sample, para4Action);
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.actionSeq_Sample, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.actionSeq_Sample, para4Action, mobileOS);    // 2nd para is the action sequence
            }
            Thread.sleep(2000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
