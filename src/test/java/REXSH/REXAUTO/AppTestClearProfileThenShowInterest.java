 ...
 */
public class AppTestClearProfileThenShowInterest extends AppTestTemplate {
    ...
    String[] TestStepList_RoleFilter_sendInterest = new String[]{"login","switch2Profile", "clearProfile","profileSaveback2dashBoard","switch2SearchRole", "Role_Send_Interest"};
    String[] TestStepList_RoleFilter_drawOther = new String[]{"login", "switch2SearchRole","Role_Filte_Interest", "Role_WithDraw_Interest"};
    ...
    public Object[][] searchRoleSendInterest() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "", "sendInterestBtn error", ""},
        };
    }
    ...
    public AppTestClearProfileThenShowInterest() throws FileNotFoundException {
    }
    ...
    @Test(dataProvider = "searchRoleSendInterest")   //  , dependsOnMethods = { "drawOther" }) // searchRoleInterest
    ...
    //@Test(dataProvider = "emptyResult" , dependsOnMethods = { "searchRoleSendInterest" })
    public void drawOther(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_RoleFilter_drawOther, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_RoleFilter_drawOther, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
