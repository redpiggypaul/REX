 ...
 */
public class AppTestShowInterestDrawAvailableDate extends AppTestTemplate {
    ...
    String[] TestStepList_RoleFilter_Interest_drawAvailableDate = new String[]{"login", "switch2SearchRole", "Role_Filte_Interest", "Role_WithDraw_InterestAvailableDate"};
    String[] TestStepList_RoleFilter_sendInterest = new String[]{"login", "switch2SearchRole", "Role_Send_Interest"};
    String[] TestStepList_recoverAvailable = new String[]{"login", "setAvailable"};

    @DataProvider
    public Object[][] emptyResult() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "Other", "regular", ""},
        };
    } // searchRoleSendInterest
    ...
    public Object[][] notAvailable() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "I'm no longer available::" + this.tomorrow, "regular", ""},
        };
    }

    public AppTestShowInterestDrawAvailableDate() throws FileNotFoundException {
    }
    ...
    @Test(dataProvider = "emptyResult", dependsOnMethods = {"drawAvailableDate"})
    public void recoverAvailable(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        ...
        try {
            ...
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_recoverAvailable, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_recoverAvailable, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
    ...
    @Test(dataProvider = "notAvailable", dependsOnMethods = {"searchRoleSendInterest"})
    public void drawAvailableDate(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_RoleFilter_Interest_drawAvailableDate, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_RoleFilter_Interest_drawAvailableDate, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
