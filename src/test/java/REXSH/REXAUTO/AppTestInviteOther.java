 ...
 */
public class AppTestInviteOther extends AppTestTemplate {
    ...
    String[] TestStepList_inviteByMail = new String[]{"login", "switch2Invite", "inviteByMail"};
    String[] TestStepList_inviteByMail_checkMailcontent = new String[]{"login", "switch2Invite", "inviteByMail_checkContent"};
    String[] TestStepList_checkMailcontent = new String[]{"login", "switch2Invite", "inviteCheckMailContent"};

    public AppTestInviteOther() throws FileNotFoundException {
    }


    @DataProvider
    public Object[][] inviteOtherMail() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "toSome001@asd.com", "regular", ""},
        };
    }

    @DataProvider
    public Object[][]
            inviteOtherMailCheckContent() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "toSome001@asd.com", "regular", "Check out PwC's Talent Exchange, a first-of-its kind marketplace that connects independent talent with exciting opportunities at PwC. With the Talent Exchange you will have the opportunities to work on high-profile projects and interesting challenges for leading brands,get direct access to PwC teams and have greater transparency into proposed project pipelines,timing and even market pricing."},
        };
    }

    @DataProvider
    public Object[][] inviteCheckMailContent() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "", "regular", "PwC's Talent Exchange  is a first-of-its kind marketplace that connects independent talent with exciting opportunities at PwC. With the Talent Exchange you will have the opportunity to work on high-profile projects and interesting challenges for leading brands, get direct access to PwC teams and have greater transparency into proposed project pipelines, time and even market pricing."},
        };
    }
    ...
    @Test(dataProvider = "inviteOtherMail")
    public void inviteOtherMail(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        ...
        try {
            ...
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_inviteByMail, para4Action);   // 2nd para is the action sequence
            }else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_inviteByMail, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(2000);
    ...
    @Test(dataProvider = "inviteOtherMailCheckContent")
    public void inviteOtherMailCheckContent(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_inviteByMail_checkMailcontent, para4Action);   // 2nd para is the action sequence
            }else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_inviteByMail_checkMailcontent, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(2000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
            date = new Date();
    ...
    @Test(dataProvider = "inviteCheckMailContent")
    public void inviteCheckMailContent(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_checkMailcontent, para4Action);   // 2nd para is the action sequence
            }else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_checkMailcontent, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(2000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
            date = new Date();
