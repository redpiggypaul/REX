 ...
 */
public class BasicDashBoardContentCheck extends AppTestTemplate {
    ...
    String[] actionSeq_All_check = new String[]{"login", "checkRoleTrack","checkRoleMatch"};


    @DataProvider
    public Object[][] interestANDRoleMatchDesc() {
        return new Object[][]{
                {this.accontFromXls, this.accontPSWFromXls, "", "regular", "Shown Interest::In order to match you with roles from the PwC Talent Exchange, we first need to know a bit about your experience and education." },
        };
    }

    public BasicDashBoardContentCheck() throws FileNotFoundException {
    }
    ...
    public void tearDown() throws Exception {
     ...
     @Test(dataProvider = "interestANDRoleMatchDesc")
    public void dashboardCheckContent(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        ...
        try {
            ...
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.actionSeq_All_check, para4Action);   // 2nd para is the action sequence
            }else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.actionSeq_All_check, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
