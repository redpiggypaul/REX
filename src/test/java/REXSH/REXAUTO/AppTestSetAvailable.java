 ...
 */
public class AppTestSetAvailable extends AppTestTemplate {
    ...
    String[] TestStepList_setAvailableDate = new String[]{"login", "setAvailableUntilDate"};
    ...
    public AppTestSetAvailable() throws FileNotFoundException {
    }
    ...
    @Test(dataProvider = "emptyResult", dependsOnMethods = {"setAvailableDate"})
    ...
    public void setAvailableDate(String theUserPara, String thePSW, String paraGroup, String targetResult, String expectResult) throws Exception {
        String nameOfMethod = Thread.currentThread().getStackTrace()[1].getMethodName();
        globalRowId++;
        String result4XLSX = "Failed";
        date = new Date();
        ...
        try {
            BaseAction theAction = new BaseAction(driver, null, 1, para4Action, cMethodPath, expectResult);  // 2nd para is for os type, last para is the excepted result str
            if (mobileOS.equalsIgnoreCase("Android")) {
                theAction.comOperationInAND((AndroidDriver) driver, this.TestStepList_setAvailableDate, para4Action);   // 2nd para is the action sequence
            } else if (mobileOS.startsWith("ios")) {
                theAction.comOperationInIOS((IOSDriver) driver, this.TestStepList_setAvailableDate, para4Action, mobileOS);     // 2nd para is the action sequence
            }
            Thread.sleep(1000);
            System.out.println("Here is the result for this case : " + theAction.getExeResult());
