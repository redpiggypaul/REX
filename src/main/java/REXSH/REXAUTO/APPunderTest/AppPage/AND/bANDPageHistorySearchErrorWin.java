 ...
 */
public class bANDPageHistorySearchErrorWin extends bANDPageTemplate {
    protected String fileName = "bHistorySearchErrorWin.json";
    ...
    private static String logSpace4thisPage = " --- bANDPageHistorySearchErrorWin --";
    ...
    public bANDPageHistorySearchErrorWin(AndroidDriver theD) throws Exception {
        super(theD);
        this.dr = theD;
    ...
    public bANDPageHistorySearchErrorWin(AndroidDriver theD, String p4L) throws Exception {
        super(theD);
        this.dr = theD;
        if (!autoOSType.toLowerCase().contains("windows")) {
            contentFilePath = new String(contentFilePath.replaceAll("\\\\", File.separator));
    ...
    public bANDPageHistorySearchErrorWin(String theFile, AndroidDriver theD, Log theLogger) throws Exception {
    ...
    public String btnOperationRoute(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperationRoute in bANDPageHistorySearchErrorWin has been called ~~~ +++");
        try {
    ...
    public String getElementContent(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The getElementContent in bANDPageHistorySearchErrorWin has been called ~~~ +++");
        try {
    ...
    public void btnOperation(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperation in bANDPageHistorySearchErrorWin has been called ~~~ +++");
        try {
    ...
    public String checkAbnormalElement(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The checkAbnormalElement in bANDPageHistorySearchErrorWin has been called ~~~ +++");
        try {
    ...
    public void textOperation(AndroidDriver driver, String eleName, String para1) throws Exception {
        System.out.println("    +++ ~~~ The textOperationn in bANDPageHistorySearchErrorWin has been called ~~~ +++");
        try {
        ...
        try {
            if (type4check.equals("ready")) {       // ready mode
                ...
                } else if (para4check.equals("title")) // check title for ready mode
                {
                    ...
                    String[] temp = this.content.getContentNameList4name(this.eleContentMap, "errorTitle");
                    if (temp.length != 1) {
                        flag = false;
                        ...
                        //TODO  compare the ((ElementMapping) temp[0]).getTextContent() and compare with the element current content to confirm the result
                        String result4ab = checkAbnormalElement(theD, "errorTitle");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            bANDPageHistorySearchErrorWin x = new bANDPageHistorySearchErrorWin("/PageXML/AND/loginPage.xml", driver, null);
