 ...
 */
public class bANDPageProfileEditClearWin extends bANDPageTemplate {
    protected String fileName = "bProfileEditClearWin.json";
    ...
    private static String logSpace4thisPage = " --- bANDPageRateDoneWin --";
    ...
    public bANDPageProfileEditClearWin(AndroidDriver theD) throws Exception {
        super(theD);
        this.dr = theD;
    ...
    public bANDPageProfileEditClearWin(AndroidDriver theD, String p4L) throws Exception {
        super(theD);
        this.dr = theD;
        if (!autoOSType.toLowerCase().contains("windows")) {
            contentFilePath = new String(contentFilePath.replaceAll("\\\\", File.separator));
    ...
    public bANDPageProfileEditClearWin(String theFile, AndroidDriver theD, Log theLogger) throws Exception {
    ...
    public void btnOperation(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperation in bANDPageRateDoneWin has been called ~~~ +++");
        try {
    ...
    public String btnOperationRoute(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperationRoute in bANDPageRateDoneWin has been called ~~~ +++");
        try {
    ...
    public void textOperation(AndroidDriver driver, String eleName, String para1) throws Exception {
        System.out.println("    +++ ~~~ The textOperationn in bANDPageRateDoneWin has been called ~~~ +++");
        try {
    ...
    public String comPageCheck(AndroidDriver theD, String type4check, String para4check) {  // type4check : ready, loading, error, temp , etc.  , para4check: target element name, all element, title element
        ...
        try {
            if (type4check.equals("ready")) {       // ready mode
                ...
                } else if (para4check.equals("message")) {
                    String[] temp = this.content.getContentNameList4name(this.eleContentMap, "message");
                    if (temp.length != 1) {
                        flag = false;
                        System.out.println("More than 1 message element found in : " + this.getClass().getSimpleName());
                        System.out.println("Please check the XML file for : " + this.getClass().getSimpleName());
                    } else {
                        flag = true;
                    }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            bANDPageProfileEditClearWin x = new bANDPageProfileEditClearWin("/PageXML/AND/loginPage.xml", driver, null);
