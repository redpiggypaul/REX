 ...
 */
public class bANDPageTaERollWin extends bANDPageTemplate {
    protected String fileName = "bTaERollWin.json";
    ...
    private static String logSpace4thisPage = " --- bANDPageTaERollWin --";
    ...
    public bANDPageTaERollWin(AndroidDriver theD) throws Exception {
        super(theD);
        this.dr = theD;
    ...
    public bANDPageTaERollWin(AndroidDriver theD, String p4L) throws Exception {
        super(theD);
        this.dr = theD;
        if (!autoOSType.toLowerCase().contains("windows")) {
            contentFilePath = new String(contentFilePath.replaceAll("\\\\", File.separator));
    ...
    public bANDPageTaERollWin(String theFile, AndroidDriver theD, Log theLogger) throws Exception {
    ...
    public String btnOperationRoute(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperationRoute in bANDPageTaERollWin has been called ~~~ +++");
        try {
    ...
    public void btnOperation(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The btnOperation in bANDPageTaERollWin has been called ~~~ +++");
        try {
    ...
    public void textOperation(AndroidDriver driver, String eleName, String para1) throws Exception {
        System.out.println("    +++ ~~~ The textOperation in bANDPageTaERollWin has been called ~~~ +++");
        try {
    ...
    public String getElementContent(AndroidDriver driver, String eleName) throws Exception {
        System.out.println("    +++ ~~~ The getElementContent in bANDPageTaERollWin has been called ~~~ +++");
        try {
    ...
    public String textOperationWithSaveInput(AndroidDriver driver, String eleName, String para1) throws Exception {
        System.out.println("    +++ ~~~ The textOperationWithSaveInput in bANDPageTaERollWin has been called ~~~ +++");
        ...
        try {
            ...
            System.out.println(logSpace4thisPage + "In bANDPageTaERollWin . textOperationWithSaveInput , Other Error appear : " + e.getCause());
            e.printStackTrace();
        } finally {
            return result;
        }
    ...
    public String rollTheRound(AndroidDriver theD, String elename, String theValue, String targetValue) throws Exception {
        int roundSco = this.roundScope; // 5 in the windows
        String result = null;
        String originalValue = theValue;
        String finalValue = targetValue;
        int step4hour = 0;
        int step4min = 0;
        WebElement theHour = null;
        WebElement theMin = null;
        boolean rollUP = true;
        try {
            if (!originalValue.contains(".")) {
                throw new REXException("wrong format of original time");
            } else {
                if (finalValue.contains(".")) {
                    if (Integer.parseInt(finalValue.split("\\.")[0]) >= 0 && Integer.parseInt(finalValue.split("\\.")[0]) <= 24) {
                        List<String> orgList = new ArrayList<String>();
                        List<String> finalList = new ArrayList<String>();
                        //    orgList = originalValue.split(".");
                        //    finalList = finalValue.split(".");
                        for (int ind = 0; ind < originalValue.split("\\.").length; ind++) {
                            orgList.add(originalValue.split("\\.")[ind]);
                        }
                        for (int ind = 0; ind < finalValue.split("\\.").length; ind++) {
                            finalList.add(finalValue.split("\\.")[ind]);
                        }
                        if (finalList.size() == 2) {
                            if ((Integer.parseInt(orgList.get(0)) == Integer.parseInt(finalList.get(0))) && (((Integer.parseInt(finalList.get(1))) == 0) || (Integer.parseInt(finalList.get(1))) == 5)) { // same hour
                                if (Integer.parseInt(orgList.get(1)) == Integer.parseInt(finalList.get(1))) {  // same min
                                    result = theValue;
                                } else { // different min
                                    //start roll the min win ele , once
                                    int step = 1;
                                    result = super.rollTheRound(theD, "min", this.byMap, this.eleContentMap, step, roundSco, rollUP);   //(AndroidDriver driver, String eleName, HashMap<String, By> inMap, HashMap<String, ElementMapping> eleMap, int stepNum, int roundRange, boolean direction)
                                }
                            } else if (Integer.parseInt(orgList.get(0)) > Integer.parseInt(finalList.get(0))) { //
                                int step = Integer.parseInt(orgList.get(0)) - Integer.parseInt(finalList.get(0));
                                rollUP = false;
                                if (step > 12) {
                                    step = 25 - step;
                                    rollUP = true;
                                }
                                result = super.rollTheRound(theD, "hour", this.byMap, this.eleContentMap, step, roundSco, rollUP);
                                if (Integer.parseInt(orgList.get(1)) != Integer.parseInt(finalList.get(1))) {  // diff min
                                    step = 1;
                                    result = super.rollTheRound(theD, "min", this.byMap, this.eleContentMap, step, roundSco, rollUP);   //(AndroidDriver driver, String eleName, HashMap<String, By> inMap, HashMap<String, ElementMapping> eleMap, int stepNum, int roundRange, boolean direction)
                                }
                            } else if (Integer.parseInt(orgList.get(0)) < Integer.parseInt(finalList.get(0))) { //
                                int step = Integer.parseInt(finalList.get(0)) - Integer.parseInt(orgList.get(0));
                                if (step > 12) {
                                    step = 25 - step;
                                    rollUP = false;
                                }
                                ...
                                throw new REXException("wrong format of target time");
                            }
                        } else {
                            throw new REXException("wrong format of target time");
                        }
                    } else {
                        throw new REXException("value out of scope(0~24) target time");
                    }
                } else {
                    if ((Integer.parseInt(finalValue) >= 0 && Integer.parseInt(finalValue) <= 24)) {
                        List<String> orgList = new ArrayList<String>();
                        for (int ind = 0; ind < originalValue.split("\\.").length; ind++) {
                            orgList.add(originalValue.split("\\.")[ind]);
                        }
                        if ((Integer.parseInt(orgList.get(0)) == Integer.parseInt(finalValue))) { // same hour
                            result = "";
                        } else if (Integer.parseInt(orgList.get(0)) > Integer.parseInt(finalValue)) { //
                            int step = Integer.parseInt(orgList.get(0)) - Integer.parseInt(finalValue);
                            rollUP = false;
                            result = super.rollTheRound(theD, "hour", this.byMap, this.eleContentMap, step, roundSco, rollUP);
                        } else if (Integer.parseInt(orgList.get(0)) < Integer.parseInt(finalValue)) { //
                            int step = Integer.parseInt(finalValue) - Integer.parseInt(orgList.get(0));
                            result = super.rollTheRound(theD, "hour", this.byMap, this.eleContentMap, step, roundSco, rollUP);
                        }
                    } else {
                        throw new REXException("wrong format of target time");
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            bANDPageTaERollWin x = new bANDPageTaERollWin("/PageXML/AND/bProfileEditPublication.xml", driver, null);
